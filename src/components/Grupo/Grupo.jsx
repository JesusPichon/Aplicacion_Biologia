...
import {
  ...
  guardarArchivoCSV,
  columnasComillas,
} from '../../services/functions/export-csv';
...
const Grupo = ({
  ...
  const handleSeleccionarGrupo = () => {
    if (showCheckBox) {
      ...
      explorar
        ? navigation.navigate('TomasExplorar', {id, nombre})
  ...
  const exportLocal = [
    {
      id: '1',
      label: 'Descargar en este dispositivo',
  ...
  const renderExportOptions = () => (
    <View
      style={[
        stylesCanales.exportOptionsContainer,
      ...
      <FlatList
        ...
        renderItem={({item}) => (
          <TouchableOpacity
            style={[
              stylesCanales.exportOptionButton,
  ...
  const handleEliminarGrupo = async grupoId => {
    try {
      ...
      await controllerPocket.ObtenerTomasFull(item.id);
  ...
  return (
    <TouchableOpacity
      style={stylesCanales.cardVertical}
      onPress={handleSeleccionarGrupo}
      onLongPress={selectionMode}>
      <ImageBackground
        source={
          isLoading ? require('../../assets/images/nature.jpg') : imageSource
        }
        resizeMode="cover"
      ...
      <View style={{flex: 1, flexDirection: 'row', width: '100%'}}>
        <View
          style={[stylesCanales.nombreView, {backgroundColor: colorPrimario}]}>
          <Text style={[stylesCanales.nombreViewText, {color: colorQuinario}]}>
            {explorar ? (
              <View style={{flexDirection: 'column'}}>
                <Text style={{color: colorQuinario, fontWeight: 'bold'}}>
                  {nombre}
                </Text>
                <Text style={{color: colorQuinario}}>
                  <Text style={{fontWeight: 'bold'}}>Publicado:</Text>{' '}
                  {formatearFecha(item.created)}
                </Text>
                <Text style={{color: colorQuinario}}>
        ...
        <View
          style={[stylesCanales.tomasView, {backgroundColor: colorTerciario}]}>
          <Text style={[stylesCanales.tomasViewText, {color: 'white'}]}>
            Tomas: {totalTomas}
          </Text>
        </View>
      </View>
