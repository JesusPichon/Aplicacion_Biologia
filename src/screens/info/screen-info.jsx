...
const InfColecta = ({navigation, route}) => {
  ...
  const [filteredData, setFilteredData] = useState({
    ...
    localidad: true,
    altitud: true,
    tipo_vegetacion: true,
    informacion_ambiental: true,
    suelo: true,
    asociada: true,
    abundancia: true,
    forma_biologica: true,
    tamano: true,
    flor: true,
    fruto: true,
    usos: true,
  ...
  const [coordsChecked, setCoordsChecked] = useState(true);
  ...
  const handleSubmitGuardar = () => {
    if (validateRequiredFields()) {
      ...
      try {
        ...
        lanzarAlerta('nuevos datos guardados correctamente');
      }
    }
  };
  ...
  const renderGeographicCoords = () => (
    <View style={{width: '100%'}}>
      <View
        ...
        <Text style={{fontWeight: 'bold', color: colorQuinario, fontSize: 15}}>
          Latitud:
        </Text>
        <TextInput
          style={{
        ...
        <View key={key}>
          <ListItem
            ...
            <ListItem.Content>
              ...
              <View style={{flexDirection: 'row', alignItems: 'center'}}>
                <Icon name="edit" size={18} color={colorQuinario} />
                {options[key] ? (
                  <Dropdown
                    data={options[key]}
                    labelField="label"
            ...
            <CheckBox
              iconType="material-community"
              checkedIcon={
                data[key] === '' ? 'checkbox-blank' : "checkbox-marked"
              }
              uncheckedIcon={
                data[key] === '' ? 'checkbox-blank' : 'checkbox-blank-outline'
              }
              containerStyle={{backgroundColor: 'transparent'}}
              disabled={data[key] === ''}
              checkedColor={colorQuinario}
